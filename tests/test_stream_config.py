...
def test_defaults():
    ...
    assert cfg.channel_name == "Weather Star"


def test_file_section_overrides_defaults():
    appcfg = types.SimpleNamespace(data={"stream": {"port": 8123, "video_encoder": "h264_rkmpp"}})
...
def test_env_overrides_file_section(monkeypatch):
    appcfg = types.SimpleNamespace(data={"stream": {"port": 8123}})
...
def test_explicit_overrides_beat_env_and_file(monkeypatch):
    appcfg = types.SimpleNamespace(data={"stream": {"port": 8123, "hls_time": 4.0}})
    _with_env(monkeypatch, PORT="9090", HLS_TIME="6")
    cfg = load_stream_config(app_config=appcfg, overrides={"port": 7000})
...
def test_unknown_file_keys_ignored():
    appcfg = types.SimpleNamespace(data={"stream": {"bogus_key": 1, "port": 9999}})
    cfg = load_stream_config(app_config=appcfg)
    assert cfg.port == 9999


def test_uses_preset():
    assert StreamConfig(video_encoder="libx264").uses_preset()
    assert StreamConfig(video_encoder="libx265").uses_preset()
    assert not StreamConfig(video_encoder="h264_rkmpp").uses_preset()
    assert not StreamConfig(video_encoder="hevc_rkmpp").uses_preset()
